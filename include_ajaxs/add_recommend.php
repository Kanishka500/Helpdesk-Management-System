...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['comrec'])) {
        ...
        //--------------------job_position--------------------------
        $jpkey=1;
        $jptitle="Recomendation by Manager";
        $lm = "Recomendation by User id : " . $userid . " Date :" . $date;
        $sql = "UPDATE complain  SET recommendation=:comrec  WHERE complain_id=:setid";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['comrec' => $comrec, 'setid' => $setid]);
