...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //--------------------jobconf------------------------------
    if (isset($_POST['invdgmrec'])) {
        $invdgmrec = $_POST['invdgmrec'];
        $status = $_POST['statusdgm'];
        $setid = $_POST['setid'];
        $userid = $_POST["userid"];
        //--------------------job_position--------------------------
        $jpkey=5;
        $jptitle="Recomendation by DGM Finance";
        $lm = "Recomendation by User id : " . $userid . " Date :" . $date;
        $sql = "UPDATE invoice  SET dgm_recommendation=:invdgmrec  WHERE complain_id=:setid";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['invdgmrec' => $invdgmrec, 'setid' => $setid]);
